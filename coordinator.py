...
def do_vote_phase(tx_id):
    ...
    for node_id, addr in participants.items():
        print(f"Phase VOTE of Node {COORD_ID} sends RPC Vote to Phase VOTE of Node {node_id}")
        ...
        print(f"[Coordinator] Node {node_id} voted {resp.vote}")
...
def do_decision_phase(tx_id, votes):
    ...
    for node_id, addr in participants.items():
        print(f"Phase DECISION of Node {COORD_ID} sends RPC GlobalDecision to Phase DECISION of Node {node_id}")
    ...
    # internal self RPC
    print(f"Phase DECISION of Node {COORD_ID} sends RPC SelfDecision to Phase DECISION of Node {COORD_ID}")
